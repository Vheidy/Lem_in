...
/*   Updated: 2020/10/28 21:59:41 by vheidy           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while ((red = get_next_line(0, &buf)))
	{
		if (buf[0] == '#')
		 ...
		 проверка на валидность координат, первый символ, и проверка на существование комнаты с таким именем */
			ft_add_in_hash_tab(name, hash_tab); // добавление комнаты в хэш-таблицу  - если по этому индексу уже ест имя, то в след лист
		else
			error();
		free(buf);
	}
